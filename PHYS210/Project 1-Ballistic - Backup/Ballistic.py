...
def EulerMethod(func, prevt, finalt, prevy, step):
    ...
    if prevt <= finalt+step:  # Checks if the time is next to last
...
def ViscosityEquation():
    ...
    AnalyticalTMax = lambertw(Wparam)*-GRAVITY*MASSLIQUID
    AnalyticalTMax -= 6*np.pi*RADIUS*VISCOSITY*INITIALVY
    AnalyticalTMax = (AnalyticalTMax/(GRAVITY*MASSLIQUID))+1
...
def DampedEquation():
    ...
    # This equation does not have an end state so instead we load ten
    # Oscillations as our max using the frequency. Then we calculated the
    # Minimum time, points, step and time span vector
    tMax = 10*np.sqrt(np.abs(SOLUTIONCOEFFICIENT))  # In seconds
    tMin = 0                                        # In seconds
    Points = 250
    Step = (tMax-tMin)/Points
    TimeSpan = np.arange(tMin, tMax, Step)
