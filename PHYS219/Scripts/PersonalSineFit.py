...
guess = (1e3*1e-7, 2*np.pi*100, np.pi, 0)
###############################################################################
def fit_function(x, RC, Freq, phase, offset):
    return 1/RC*1/Freq*0.05*np.cos(Freq*x+phase)+offset
...
plt.title('Initial Guess Fit for Integrator Op Amp with Sine Wave')
...
params_names = ['RC Time Constant', 'Frequency', 'Phase', 'Voffset']
print('\nFit parameters')
...
plt.xlabel('{} [{}]'.format(x_name, x_unit))
plt.ylabel('{} [{}]'.format(y_name, y_unit))
plt.title('Fitted Guess for Integrator Op Amp with Sine Wave')
plt.legend(loc='best', numpoints=1)
